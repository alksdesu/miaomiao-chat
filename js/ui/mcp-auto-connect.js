...
export async function autoConnectMCPServers(options = {}) {
    const { showNotifications = true, delayBetweenConnections = 100 } = options;

    logger.debug('[MCP AutoConnect] 🚀 开始自动连接 MCP 服务器...');

    if (!state.mcpServers || state.mcpServers.length === 0) {
        logger.debug('[MCP AutoConnect] 没有配置的 MCP 服务器');
    ...
    for (const server of state.mcpServers) {
        ...
        if (server.enabled === false) {
            logger.debug(`[MCP AutoConnect] 跳过已禁用: ${server.name}`);
            results.total--;
            continue;
        }

        try {
            logger.debug(`[MCP AutoConnect] 正在连接: ${server.name} (${server.type})`);

            const result = await mcpClient.connect(server);

            if (result.success) {
                results.connected++;
                logger.debug(`[MCP AutoConnect] 成功连接: ${server.name}`);
                ...
                logger.debug(`[MCP AutoConnect] 服务器 ${server.name} 提供 ${tools.length} 个工具`);
            } else {
                results.failed++;
                results.errors.push({
                    serverId: server.id,
                ...
                logger.error(`[MCP AutoConnect] ❌ 连接失败: ${server.name}`, result.error);
            }
            ...
            logger.error(`[MCP AutoConnect] ❌ 连接异常: ${server.name}`, error);
        }
    }
    ...
    logger.debug('[MCP AutoConnect] 自动连接完成:', results);
    ...
    if (results.connected > 0) {
        ...
        try {
            ...
            logger.error('[MCP AutoConnect] 加载工具状态失败:', error);
        }
    }
...
export function initMCPAutoConnect(startupDelay = 500) {
    logger.debug('[MCP AutoConnect] 初始化自动连接...');

    // 延迟执行，确保其他模块已经初始化
    setTimeout(async () => {
        logger.debug('[MCP AutoConnect] 执行自动连接...');
...
logger.debug('[MCP AutoConnect] 模块已加载');
