// vendored from https://github.com/vercel/streamdown/tree/main/packages/remend (Apache-2.0)
// version: 1.3.0
// downloaded: 2026-05-29
// source: https://unpkg.com/remend@1.3.0/dist/index.js
// exports: default (remend fn), isWithinCodeBlock, isWithinLinkOrImageUrl, isWithinMathBlock, isWordChar
...
var O = (n, r, e) =>
        r in n
            ? cn(n, r, { enumerable: true, configurable: true, writable: true, value: e })
            : (n[r] = e),
    I = (n, r) => {
...
var c = (n, r) => {
    ...
    hn = (n, r) => {
        let e = n.substring(r, r + 3) === '```',
            i = r > 0 && n.substring(r - 1, r + 2) === '```',
            s = r > 1 && n.substring(r - 2, r + 1) === '```';
        return e || i || s;
    },
    L = (n) => {
        let r = 0;
...
var mn = /^(\s*(?:[-*+]|\d+[.)]) +)>(=?\s*[$]?\d)/gm,
    E = (n) =>
        !n || typeof n != 'string' || !n.includes('>')
            ? n
            : n.replace(mn, (r, e, i, s) => (c(n, s) ? r : `${e}\\>${i}`));
var M = /(\*\*)([^*]*\*?)$/,
    N = /(__)([^_]*?)$/,
    y = /(\*\*\*)([^*]*?)$/,
    R = /(\*)([^*]*?)$/,
...
`;
    ...
    On = (n, r, e) => (!r || d.test(r) || c(n, e) || f(n, e) ? true : p(n, e, '*')),
    V = (n) => {
...
var en = (n, r) =>
        (r >= 2 && n.substring(r - 2, r + 1) === '```') ||
        (r >= 1 && n.substring(r - 1, r + 2) === '```') ||
        (r <= n.length - 3 && n.substring(r, r + 3) === '```'),
    Nn = (n) => {
        let r = 0,
            e = false;
...
var S = (n) => n !== false,
    ...
    Xn = [
        {
            handler: { name: 'singleTilde', handle: tn, priority: u.SINGLE_TILDE },
            optionKey: 'singleTilde'
        },
        {
            handler: { name: 'comparisonOperators', handle: E, priority: u.COMPARISON_OPERATORS },
            optionKey: 'comparisonOperators'
        },
        { handler: { name: 'htmlTags', handle: x, priority: u.HTML_TAGS }, optionKey: 'htmlTags' },
        {
            handler: { name: 'setextHeadings', handle: ln, priority: u.SETEXT_HEADINGS },
            optionKey: 'setextHeadings'
        },
        {
            handler: { name: 'links', handle: B, priority: u.LINKS },
            optionKey: 'links',
            earlyReturn: (n) => n.endsWith('](streamdown:incomplete-link)')
        },
        {
            handler: { name: 'boldItalic', handle: V, priority: u.BOLD_ITALIC },
            optionKey: 'boldItalic'
        },
        { handler: { name: 'bold', handle: j, priority: u.BOLD }, optionKey: 'bold' },
        {
            handler: {
        ...
        {
            handler: { name: 'inlineCode', handle: nn, priority: u.INLINE_CODE },
            optionKey: 'inlineCode'
        },
        {
            handler: { name: 'strikethrough', handle: an, priority: u.STRIKETHROUGH },
            optionKey: 'strikethrough'
        },
        { handler: { name: 'katex', handle: rn, priority: u.KATEX }, optionKey: 'katex' },
        {
            handler: { name: 'inlineKatex', handle: sn, priority: u.INLINE_KATEX },
    ...
    zn = (n) => {
        ...
        return Xn.filter(({ handler: i, optionKey: s }) =>
            i.name === 'links'
                ? S(n == null ? void 0 : n.links) || S(n == null ? void 0 : n.images)
                : i.name === 'inlineKatex'
                  ? Fn(n == null ? void 0 : n.inlineKatex)
                  : S(n == null ? void 0 : n[s])
        ).map(({ handler: i, earlyReturn: s }) =>
            i.name === 'links'
                ? {
                      handler: k(I({}, i), { handle: (o) => B(o, r) }),
                      earlyReturn: r === 'protocol' ? s : void 0
                  }
                : { handler: i, earlyReturn: s }
        );
    },
    vn = (n, r) => {
        var t;
        if (!n || typeof n != 'string') return n;
        let e = n.endsWith(' ') && !n.endsWith('  ') ? n.slice(0, -1) : n,
            i = zn(r),
            s = ((t = r == null ? void 0 : r.handlers) != null ? t : []).map((a) => {
                var l;
                return {
            ...
            o = [...i, ...s].sort((a, l) => {
                var _, P;
                return (
                    ((_ = a.handler.priority) != null ? _ : 0) -
                    ((P = l.handler.priority) != null ? P : 0)
                );
            });
        for (let { handler: a, earlyReturn: l } of o)
            if (((e = a.handle(e)), l != null && l(e))) return e;
        return e;
    },
    $e = vn;
export {
