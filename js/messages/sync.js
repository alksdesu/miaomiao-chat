...
import {
    createMessage, createMeta, Role, PartType, MediaKind, ToolState,
    textPart, thinkingPart, mediaPart, toolCallPart,
    filterParts, isNewFormat,
} from './schema.js';
...
function simpleHash(str) {
    ...
    const sample = len > 256
        ? str.slice(0, 64) + str.slice(-64) + len
        : str;
    let hash = 0;
...
/**
 * 从旧格式 contentParts 构建新格式 parts 数组
...
function buildPartsFromOldParams(opts) {
    const {
        textContent, thinkingContent, thinkingSignature,
        thoughtSignature, contentParts, toolCalls,
        thinkingBlocks, thinkingSignatures,
    } = opts;

    let parts = [];

    // 检查 contentParts 中是否已包含 thinking
    const contentPartsHasThinking = Array.isArray(contentParts) &&
        contentParts.some(p => p.type === 'thinking' && p.text);
    ...
    if (toolCalls && toolCalls.length > 0) {
        for (const tc of toolCalls) {
            ...
            if (tc.status === 'completed' && tc.result != null) {
                ...
                tcPart.result = { content: typeof tc.result === 'string' ? tc.result : JSON.stringify(tc.result) };
...
function buildMeta(opts) {
    const { streamStats, encryptedContent, reasoningItemId, thoughtSignature, thinkingSignature, groundingMetadata } = opts;
...
function convertReplyToNewParts(reply) {
    ...
    if (reply.contentParts && reply.contentParts.length > 0) {
        parts.push(...convertOldContentParts(reply.contentParts));
...
export function saveAssistantMessage(options) {
    ...
    const meta = buildMeta({
        streamStats, encryptedContent, reasoningItemId, thoughtSignature, thinkingSignature, groundingMetadata,
    });

    // 会话切换检查
    if (sessionId && sessionId !== state.currentSessionId) {
        console.warn(`[sync] 会话已切换（${sessionId} → ${state.currentSessionId}），将消息保存到原会话`);
    ...
    eventBus.emit('messages:assistant-added', { index: messageIndex });
...
function findMergeTarget() {
    ...
    if (domIndexStr !== undefined) {
        ...
        if (!Number.isNaN(idx) && idx >= 0 && idx < state.messages.length &&
    ...
    for (let i = state.messages.length - 1; i >= 0; i--) {
        ...
        if (isNewFormat(msg)) {
            const hasContent = msg.parts?.some(p =>
                p.type === PartType.TEXT || p.type === PartType.THINKING || p.type === PartType.MEDIA
            );
        ...
        // 旧格式兼容（迁移后不应触发）
        console.warn('[Sync] findMergeTarget 命中旧格式消息:', i);
        ...
        const isToolCallsOnly = (Array.isArray(msg.tool_calls) && msg.tool_calls.length > 0 ||
            ...
            !hasTextContent && !hasContentParts && !hasThinking;
...
function mergeContinuation(index, newParts, newMeta, _toolCalls) {
    ...
    console.log(`[saveAssistantMessage] Continuation 模式：更新消息 #${index}`);
    ...
    const prevTextParts = filterParts(prev.parts, PartType.TEXT).filter(p => p.text !== '(调用工具)');
    const newTextParts = newParts.filter(p => p.type === PartType.TEXT && p.text !== '(调用工具)');
    ...
    const mergedParts = [
        ...prevThinkingParts, ...newThinkingParts,
        ...prevTextParts, ...newTextParts,
        ...deduplicateMediaParts([...prevMediaParts, ...newMediaParts]),
        ...prevToolParts, ...newToolParts,
    ];
    ...
    if (newMeta.stats) {
        ...
        if (prevStats?.isPartial) {
            ...
            if (currentTokens < prevTokens) {
                ...
                const ttft = (prevStats.ttft && prevStats.ttft !== '-') ? prevStats.ttft : newMeta.stats.ttft;
                const totalTimeNum = parseFloat(newMeta.stats.totalTime);
                const ttftNum = parseFloat(ttft);
                const genTime = (Number.isFinite(totalTimeNum) && Number.isFinite(ttftNum)) ? (totalTimeNum - ttftNum) : NaN;
                mergedMeta.stats = {
                    ...newMeta.stats, ttft, tokens: totalTokens,
                    tps: Number.isFinite(genTime) && genTime > 0 ? (totalTokens / genTime).toFixed(1) : newMeta.stats.tps,
                };
            } else {
                mergedMeta.stats = { ...newMeta.stats, ttft: (newMeta.stats.ttft && newMeta.stats.ttft !== '-') ? newMeta.stats.ttft : prevStats.ttft };
            }
        } else {
...
function saveToBackgroundSession(sessionId, parts, meta, allReplies, selectedReplyIndex, ts) {
    ...
    import('../state/storage.js').then(async ({ loadSessionMessages, saveSessionMessages, saveSessionToDB }) => {
        try {
            ...
            console.log(`[sync] 消息已保存到后台会话: ${sessionName}`);
            eventBus.emit('ui:notification', { message: `消息已保存到会话"${sessionName}"`, type: 'info' });
        } catch (e) {
            console.error('[sync] 保存后台会话消息失败:', e);
        }
    });
}
...
export function saveErrorMessage(errorData, httpStatus = null, renderHumanizedError) {
    ...
    eventBus.emit('messages:error-added', { index: messageIndex });
    ...
    for (let i = state.messages.length - 1; i >= 0; i--) {
        ...
        if (msg.parts) {
            ...
            if (tcPart) {
                ...
                tcPart.result = status === 'completed'
                    ? { content: typeof result === 'string' ? result : JSON.stringify(result) }
                    : { error: typeof result === 'string' ? result : result?.message || 'unknown error' };
                state.sessionDirty = true;
                eventBus.emit('state:message-updated', { index: i, msg });
                console.log('[Sync] 工具调用结果已保存到消息 #' + i + ' (parts)');
                triggerSave();
                return;
            }
        }
        ...
        if (msg.toolCalls && Array.isArray(msg.toolCalls)) {
            console.warn('[Sync] updateToolCallResult 命中旧格式 toolCalls[]，消息未迁移:', i);
            const tcIdx = msg.toolCalls.findIndex(tc => tc.id === toolId);
            if (tcIdx !== -1) {
                msg.toolCalls[tcIdx] = {
                    ...msg.toolCalls[tcIdx],
                    status, result: status === 'completed' ? result : null,
                    error: status === 'failed' ? result : null,
                    completedAt: Date.now(),
                };
                state.sessionDirty = true;
                console.log('[Sync] 工具调用结果已保存到消息 #' + i + ' (toolCalls)');
                triggerSave();
                return;
            }
        }
...
function triggerSave() {
    ...
    }).catch(err => console.error('[Sync] 加载会话保存模块失败:', err));
}
